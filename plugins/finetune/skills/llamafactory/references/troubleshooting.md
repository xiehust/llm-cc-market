...
**Problem**: `bash: uv: command not found` or `/home/ubuntu/lmf-env/bin/uv: No such file or directory` when running `uv` commands via SSH.
...
ssh -i PEM ubuntu@IP "~/.local/bin/uv pip install PACKAGE --python ~/lmf-env/bin/python"

# Option 2: Source bashrc first
ssh -i PEM ubuntu@IP "source ~/.bashrc && uv pip install PACKAGE --python ~/lmf-env/bin/python"
```
Note: `uv` is NOT inside `~/lmf-env/bin/`. It is always at `~/.local/bin/uv`.
