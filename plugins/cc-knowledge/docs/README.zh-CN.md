...
- **智能门控** — 只对有实质内容的会话进行知识沉淀（有文件编辑、命令错误或用户纠正）
...
钩子脚本（`scripts/session-end-cultivate.js`）在每次会话结束时触发，应用**确定性启发式门控**（门控本身不消耗 LLM 调用）：

**门控条件（全部满足）：**
1. 会话有 ≥8 条用户消息
2. 至少满足以下之一：
   - 编辑了文件（使用了 Edit/Write 工具）
   - Bash 命令返回错误
   - 用户纠正了 Claude（"不对"、"错了"、"用 X 代替"等）

门控通过后，脚本：
1. 从最近消息中提取主题提示
2. 启动 `claude -p` 运行沉淀引擎
3. 在 `~/.claude/cc-knowledge-pending/` 写入待处理标记
...
检查门控条件：
- 你的会话需要 ≥8 条用户消息
- 并且至少有一次文件编辑、bash 错误或用户纠正
- 随时使用 `/cc-knowledge:cultivate` 进行手动提取
