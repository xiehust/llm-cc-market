...
**Architecture diagram (optional).** If the topic has ≥2 named components with clear relationships, lead this section with an architecture diagram generated by the fireworks-tech-graph skill. Embed it right after the section's opening paragraph:

```markdown
![<topic> architecture](diagrams/<YYYY-MM-DD>/architecture.png)
*Figure 1: <one-line caption naming the layers/components>.*
```

Every node in the diagram must map to a real component named in the wiki — never invent boxes to fill it out. If the solution is a single flat idea with no components, skip the diagram. See [diagrams.md](diagrams.md) for the full generation/embedding workflow.
...
Only include if the topic clearly shows feedback loops (e.g., one decision feeding another). 2-3 paragraphs explaining the loop.

**Flow diagram (optional).** If the content describes an ordered process (≥3 steps), a request/response path, or a feedback loop, render it as a `flowchart` or `data flow` diagram via fireworks-tech-graph instead of (or alongside) ASCII art:

```markdown
![<process> flow](diagrams/<YYYY-MM-DD>/<slug>-flow.png)
*Figure N: <one-line caption naming the steps>.*
```

ASCII is an acceptable fallback when diagram generation is unavailable. See [diagrams.md](diagrams.md).
